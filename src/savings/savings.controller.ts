...
export class SavingsController {
    ...
    async createSaving(
        ...
        const saving = await this.savingsService.createSaving(
            userId,
            createSavingDTO,
        );
    ...
    @Get('')
    @UseGuards(AuthGuard)
    async getAllSavings(@Req() req: Request) {
        const userId = req['user']._id;
    ...
    async deleteSavings(
        ...
        const userId = req['user']._id;
        const deletedSavings = await this.savingsService.deleteSaving(
            userId,
            savingsId,
        );
    ...
    async updateIncomeInfo(
        ...
        const userId = req['user']._id;
        const savings = await this.savingsService.updateSaving(
            userId,
            savingsId,
            updateSavingDTO,
        );

        return ResponseHandler(HttpStatus.OK, 'Savings Updated', savings);
    }
}
